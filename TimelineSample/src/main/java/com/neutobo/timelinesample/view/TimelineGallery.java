...
/**
 * This class represents a horizontal view centered gallery kind of view.
 * It is a blatant copy of the original (deprecated) Gallery from Android.
 * I found it on StackOverflow.
 * It has bugs and it seems to be copied so its real origin is unknown.
 * I copied it from what was called "EcoGallery" then made some changes (notably scrolling report, smooth scroll to, etc.)
 * Anyway, I changed a few things, but it's still kinda lame. Any future dev should seek a better method.
 * I'd rather use a ViewPager (with a modification to be able to see more than one page).
 * Or a horizontal list view with some "snap to center" code like this "Gallery".
 * Use at your own risk :)
 * The license is: do whatever you want, but keep Google's copyright where it belongs.
 * (Although I have the feeling they don't want to be remembered as the ones who wrote the original gallery…
 *
 * This class was modified by Martin Marconcini <marconcini@gmail.com>
 *
 * Changes from EcoGallery:
 *
 * Slower animation to make it feel much better.
 * Added smooth scroll to (that sometimes doesn't work ok, but for short distances is fine).
 * Made fling sensitivity customizable (changes acceleration).
 * Fixed some NPE crashes.
 *
 * <b>WARNING: </b> Google's gallery was (and still is) bad and buggy. It solved a UI problem in a very inelegant
 * way, full of hacks (which are still visible here). Unless you really have to use this, I'm sure there are better
 * alternatives. Using a ViewPager is probably better or even a HorizontalScrollView.
 *
 * The code, however, does work (as it did the original Gallery) and thanks to modifications (EcoGallery and such)
 * it does a better job at recycling views, something the original Gallery didn't even do (yes, LOL).
 *
 * Still, due to the architecture of the whole gallery (firing runnables and such) the room for error is huge and
 * some handsets will behave strangely. One day, hopefully, Google will release a proper component (or someone with
 * enough time will write it) :)
 *
 * My advice, don't copy and paste without reading the code, understanding what it does, it's the best way to learn.
 * May look weird at first, but it's really not rocket science. Read about each line, ask yourself What does this do?
 * What happens if I change this? Why? ;)
 *
 * Good luck and Thanks to all those who contributed.
 *
 * If you're thinking about filing "bug reports" to me, think twice as I might not have time to fix this (it's Google's
 * code after all) ;)
 *
 *
 * *
 */
public class TimelineGallery extends TimelineGalleryAbsSpinner implements GestureDetector.OnGestureListener {
	public static final int FLING_VELOCITY_DIVIDER = 3;// MUST BE > 0
	/**
	...
	public TimelineGallery(Context context) {
		this(context, null);
	}
	public TimelineGallery(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.timelineGalleryStyle);
	}
	public TimelineGallery(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		mBroken = true;
		mGestureDetector = new GestureDetector(context, this);
		mGestureDetector.setIsLongpressEnabled(true);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TimelineGallery, defStyle, 0);
		int index = 0;
		if (a != null) {
			index = a.getInt(R.styleable.TimelineGallery_gravity, -1);
		}
		if (index >= 0) {
			setGravity(index);
		}
		...
		if (a != null) {
			animationDuration = a.getInt(R.styleable.TimelineGallery_animationDuration, -1);
		}
		...
		int spacing = 0;
		if (a != null) {
			spacing = a.getDimensionPixelOffset(R.styleable.TimelineGallery_spacing, 0);
		}
		setSpacing(spacing);
		float unselectedAlpha = a.getFloat(R.styleable.TimelineGallery_unselectedAlpha, 0.5f);
	...
	protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
		/*
		 * Gallery expects TimeLine.LayoutParams.
		 */
...
//		}
	...
	private void setSelectionToCenterChild() {
		...
		for (int i = getChildCount() - 1; i >= 0; i--) {
			...
			if (child != null) {
				childClosestEdgeDistance = Math.min(Math.abs(child.getLeft() - galleryCenter),
...
//		Logger.d("###TIMELINE2###: layout");
		...
		// All views go in recyclers while we are in layout
		recycleAllViews();
		// Clear out old views
	...
	/**
	 * Obtain a view, either by pulling an existing view from the recycle or by
	 ...
	 * @param offset   Offset from the selected position
	 * @param x        X-coordinate indicating where this view should be placed.
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		/*
		 * Now is a good time to tell our parent to stop intercepting our events!
		 * The user has moved more than the slop amount, since GestureDetector
		 * ensures this before calling this method. Also, if a parent is more
		 * interested in these touch events than we are, it would have
	...
	private void updateSelectedItemMetadata() {
		...
		// We un focus the old child down here so the above hasFocus check
		// returns true
	 ...
	 *
	 * @param gravity the view gravity
	...
	public void smoothScrollTo(int currentPosition) {
		...
		if (getSelectedView() != null && getSelectedView().getWidth() > 0) {
			int width = getSelectedView().getWidth();
			int distance;
			int posDelta = Math.abs(currentPosition - mSelectedPosition);
			distance = (posDelta * mSpacing) + (posDelta * width);
//			Logger.d("###TIMELINE###.smoothScrollTo()->distance: " + distance);
			if (mSelectedPosition > currentPosition) {
				mFlingRunnable.startUsingDistance(distance, 750);
			} else {
				mFlingRunnable.startUsingDistance(-1 * distance, 750);
			}
		}
	}
	/**
	...
	private class FlingRunnable implements Runnable {
		...
		public void startUsingVelocity(int initialVelocity) {
			...
			mScroller.fling(initialX, 0, initialVelocity / FLING_VELOCITY_DIVIDER, 0, 0, Integer.MAX_VALUE, 0, Integer.MAX_VALUE);
			post(this);
		}
		public void startUsingDistance(int distance, final int duration) {
			if (distance == 0)
				return;
			startCommon();
			mLastFlingX = 0;
